...
def single_inference(model, image_dict, return_offset=False):
    with torch.no_grad():
        ...
        image = image.cuda()
        trimap = trimap.cuda()
        alpha_pred, info_dict, _ = model(image, trimap)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image-dir', type=str, default='', help="input image dir")
    parser.add_argument('--trimap-dir', type=str, default='', help="input trimap dir")
    ...
    for image_name in os.listdir(args.image_dir):
        ...
        all_images[image_name] = image_dict

    inference_start = time.perf_counter()
    for image_name, image_dict in all_images.items():
    ...
    print("Inference time: ", (inference_end - inference_start) / len(all_images))