...
class Trainer(object):
    ...
    def build_model(self):
        ...
        self.D = networks.get_discriminator().cuda()

        self.criterionGAN = networks.get_GANLoss().cuda()
    ...
    def restore_model(self, resume_checkpoint):
        ...
        if 'state_dict_D' in checkpoint.keys():
        ...
        if not self.train_config.reset_lr:
            ...
            if 'opt_state_dict_D' in checkpoint.keys():
                try:
                    self.D_optimizer.load_state_dict(checkpoint['opt_state_dict_D'])
                except ValueError as ve:
                    self.logger.error("{}".format(ve))
            ...
            if 'lr_state_dict_D' in checkpoint.keys():
                try:
                    self.D_scheduler.load_state_dict(checkpoint['lr_state_dict_D'])
                except ValueError as ve:
                    self.logger.error("{}".format(ve))
        else:
            self.G_scheduler = lr_scheduler.CosineAnnealingLR(self.G_optimizer,
    ...
    def train(self):
        ...
        for step in range(start, self.train_config.total_step + 1):
            ...
            real_image = image_dict['real_image']
            ...
            if step < self.train_config.warmup_step and self.train_config.resume_checkpoint is None:
                ...
                cur_D_lr = utils.warmup_lr(self.train_config.G_lr, step + 1, self.train_config.warmup_step)
                utils.update_lr(cur_D_lr, self.D_optimizer)
                ...
                cur_D_lr = self.D_scheduler.get_lr()[0]

            """===== Forward G ====="""
            alpha_pred, info_dict ,embedding = self.G(image, trimap) # info_dict: intermediate feature of networks like attention
            weight = utils.get_unknown_tensor(trimap)

            if self.train_config.use_da == True:
            # =======================================================
            ## Synthetic
                ...
                feature_input = embedding.detach().cpu()
                pred_syn = self.D(feature_input.cuda())
                self.loss_D_syn = self.criterionGAN(pred_syn, False)

                #print(image.shape)
                #print(real_image.shape)
                _, _, real_embedding = self.G(real_image,  trimap)
                
                #inp = torch.cat((image, trimap), dim=1)
                # self.encoder  = encoders.__dict__[self.model_config.arch.encoder()]()
                #embedding, _ = self.G.encoder(inp)

                ## Real
                real_feature_input = real_embedding.detach().cpu()
                pred_real = self.D(real_feature_input.cuda())
            ...
            if (self.train_config.use_da == True) & (self.train_config.dis_weight >0) :
                pred_syn = self.D(embedding.cuda())
                self.loss_dict['dis'] = self.criterionGAN(pred_syn, True) * self.train_config.dis_weight
                #print('da_success')


            for loss_key in self.loss_dict.keys():
                if self.loss_dict[loss_key] is not None and loss_key in ['rec', 'comp', 'smooth_l1', 'grad', 'gabor','dis']:
            ...
            if step % self.log_config.logging_step == 0:
                ...
                if CONFIG.dist:
                    ...
                    self.loss_D = utils.reduce_tensor_dict(self.loss_D, mode='mean')
                ...
                log_info += "total_loss :{:4f}".format(loss)
                ...
                #log_info += "dis:{:6f}}".format(self.loss_dict['dis'])
            ...
            if ((step % self.train_config.val_step) == 0 or step == self.train_config.total_step):# and step > start:
                ...
                with torch.no_grad():
                    for image_dict in self.test_dataloader:
                        ...
                        alpha_pred, info_dict,embedding = self.G(image, trimap)
                ...
                if (step % self.log_config.checkpoint_step == 0 or step == self.train_config.total_step) \
                    ...
                    if self.train_config.use_da  == True:
                        self.save_model("model_"+str(step), step, loss)
    ...
    def save_model(self, checkpoint_name, iter, loss):
        ...
        torch.save({
            ...
            'opt_state_dict_D': self.D_optimizer.state_dict(),
            'lr_state_dict_D': self.D_scheduler.state_dict()
