...
def compute_gradient_loss(pred, target, trimap_input, prior='trimap'):

    if prior is 'PR':
        trimap = np.full_like(trimap_input,128)
    else:
        trimap = trimap_input
...
def compute_connectivity_error(pred, target, trimap_input, step,  prior='trimap'):
...
def compute_mse_loss(pred, target, trimap_input, prior='trimap'):
...
def comput_sad_loss(pred, target, trimap_input, prior='trimap'):
    if prior is 'PR':
        trimap = np.full_like(trimap_input,128)
    else:
        trimap = trimap_input
