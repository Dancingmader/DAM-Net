...
class ModelProxy(object):
    ...
    def __str__(self):
        return (
            "version: {}; SAD: {}; MSE: {}; Grad: {}; Conn: {}; use_trimap: {}".format(self.version, self.SAD, self.MSE,
                                                                                       self.Grad, self.Conn,
                                                                                       self.use_trimap))
    ...
    def test_v2(self):
        ...
        with torch.no_grad():
            for image_dict in self.test_dataloader:
                ...
                if CONFIG.prior is 'PR':
                    pass
                    # 'trimap': (trimap[-1, ...].data.cpu().numpy() * 255).astype(np.uint8),
                else:
                ...
                for cnt in range(image.shape[0]):
                    ...
                    if CONFIG.prior is 'PR':
                        test_trimap = (trimap[cnt, 0, ...].data.cpu().numpy() * 255).astype(np.uint8)
                    else:
                        test_trimap = trimap[cnt, 0, ...].data.cpu().numpy()
                    test_trimap = test_trimap[:h, :w]

                    mse_loss += compute_mse_loss(test_pred, test_alpha, test_trimap, prior=CONFIG.prior)
                    sad_loss += comput_sad_loss(test_pred, test_alpha, test_trimap, prior=CONFIG.prior)[0]
                    if not self.test_config.fast_eval:
                        conn_loss += compute_connectivity_error(test_pred, test_alpha, test_trimap, 0.1,
                                                                prior=CONFIG.prior)
                        grad_loss += compute_gradient_loss(test_pred, test_alpha, test_trimap, prior=CONFIG.prior)

                    if self.test_config.out_path is not None:
                        cv2.imwrite(os.path.join(self.test_config.out_path, os.path.splitext(name[cnt])[0] + ".png"),
                                    test_pred)
                    if self.test_config.viz_dir is not None:
                        ...
                        gt = cv2.merge(
                            (test_alpha.astype(np.uint8), test_alpha.astype(np.uint8), test_alpha.astype(np.uint8)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--config', type=str, default='config/DAM-Net.toml')
