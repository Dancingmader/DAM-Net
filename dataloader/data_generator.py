...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if self.phase == "train":
            ...
            real_image = sample['real_image'][:, :, ::-1].transpose((2, 0, 1)).astype(np.float32) / 255.
            sample['real_image'] = torch.from_numpy(real_image).sub_(self.mean).div_(self.std)
...
class RandomCropV2(object):
    ...
    def __call__(self, sample):
        ...
        fg, alpha, trimap, bg, name = sample['fg'], sample['alpha'], sample['trimap'], sample['bg'], sample[
            'image_name']
        h, w = trimap.shape
        ...
        bg_crop = cv2.resize(bg_crop, self.output_size, self.output_size,
                             interpolation=maybe_random_interp(cv2.INTER_CUBIC))
        sample['bg'] = bg_crop
...
class DataGenerator(Dataset):
    def __init__(self, data, phase="train", test_scale="resize"):
        ...
        if self.phase == "train":
            ...
            self.real_image_num = len(self.real_image)
            self.merged = []
            self.trimap = []
        else:
            self.fg = []
    ...
    def __getitem__(self, idx):
        if self.phase == "train":
            ...
            # if fg.shape[0] == 640 or fg.shape[1] == 640:
            ...
            real_image = cv2.imread(self.real_image[idx % self.real_image_num])
            # print("fg shape = {}, alpha shape = {}".format(fg.shape,alpha.shape))

            # if CONFIG.data.augmentation and random.random() > 0.9:
            ...
            sample = {'fg': fg, 'alpha': alpha, 'bg': bg, 'image_name': image_name,
                      'pr_fn': self.fg[idx % self.fg_num].replace("image", "trimap"),
                      'pupil_fn': self.pupil_bg[(idx + random.randint(0, 100)) % self.pupil_bg_num],
                      'real_image': real_image, 'idx': idx}
            ...
            # eal_image = cv2.imread(self.real_image[idx % self.real_image_num])
